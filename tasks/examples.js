...
async function main() {
  ...
  const packages = await Promise.all(
    packageNames
      ...
      .map(async name => {
        ...
        return {
          ...descriptor,
          examples: examples.map(_name => ({
            filepath: path.join(examplesDir, _name),
            name: _name,
          })),
        };
      })
  ...
  const links = packagesWithExamples.reduce((html, pkg) => {
    const _links = pkg.examples.reduce((acc, example) => {
    ...
    return (
      ...
      `<section>
  <header>
    <h2><pre style="display:inline;"><code>@chancethedev/${pkg.name}</code></pre></h2>
  </header>
  <ul>
    ${_links}
  </ul>
...
<head>
  ...
  <title>Chance the Developer Design System</title>
